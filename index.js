...
prefix = "!"
name = "~ MEDO | MEU PAU"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM GAY😁"
rmenu = "Medo:)"
limitt = 10
ban = []
userpremium = ["558899704203@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do Medo digite ${prefix}daftar`,
			...
			const ownerNumber = ["558899704203@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do Meu pau:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ marcando todo mundo fds *${pushname2}* ✪══`+ teks +'╚═══〘 MEDO 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
